...
(function checkEnv() {
  if (!env.stripe.secretKey) {
    console.warn(
      "[Truststay] STRIPE_SECRET_KEY not set — Stripe checkout will not work. " +
  ...
  if (!env.stripe.cityPassPriceId) {
    console.warn(
      "[Truststay] CITY_PASS_PRICE_ID not set — Stripe checkout will not work. " +
  ...
  if (!env.stripe.webhookSecret) {
    console.warn(
      "[Truststay] STRIPE_WEBHOOK_SECRET not set — webhook signature validation will be skipped. " +
  ...
  if (!env.unlock.signingKey) {
    console.warn(
      "[Truststay] UNLOCK_SIGNING_KEY not set — unlock cookie will not be HMAC-signed. " +
  ...
  if (env.app.isProduction && !env.app.url) {
    console.warn(
      "[Truststay] NEXT_PUBLIC_APP_URL not set in production — Stripe redirect URLs will be " +
